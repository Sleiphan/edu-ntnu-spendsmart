...
/**
 * The User class represents a user in the banking system. It inherits from the Personalia class.
 */
public class User extends Personalia {
  ...
  public User(Account[] accounts, Invoice[] invoices,
    ...
    this.invoices =
        invoices != null ? Arrays.stream(invoices).collect(Collectors.toList()) : new ArrayList<>();
    this.budget = budget;
   ...
   * @throws java.util.NoSuchElementException if no account with the specified account number is
   *                                          found
   */
  ...
  /**
   * Returns the total income or expenses of all accounts in the last 30 days, depending on the
    ...
    return ApplicationObjects.formatCurrency(this.transactions
        ...
        .filter(transaction ->
            incomeOrExpenses ? accountNumbers.get()
                ...
                : accountNumbers.get().anyMatch(
                    accountNumber -> accountNumber.equals(transaction.getFromAccountNumber())))
        .filter(transaction ->
  ...
  /**
   * Returns the total income or expenses of all accounts in the last year, depending on the value
   * of the parameter.
   *
   * @param incomeOrExpenses true to return total income, false to return total expenses
   * @return a string representation of the total income or expenses
