 ...
 * reading and writing data to text files. The class ensures the necessary save files are created if
 * they do not exist.
 */
public class FileManagement {
  ...
  public static User readUser(String userId) throws IOException {
    ...
    return new User(accounts, invoices, user.getLoginInfo(), user.getEmail(), user.getLastName(),
        user.getFirstName(), transactions, budget);
  }
   ...
   * @throws RuntimeException if an IOException occurs while deleting the user's data from the data
   *                          file.
   */
  ...
  /**
   * Returns an array of the specified number of the most recent transactions for the specified
   * user.
   *
   * @param userId the ID of the user to retrieve transactions for.
   * @param amount the number of transactions to retrieve.
   * @return an array of the specified number of the most recent transactions for the specified
   * user.
   * @throws IOException if an error occurs while reading from the data file.
   */
