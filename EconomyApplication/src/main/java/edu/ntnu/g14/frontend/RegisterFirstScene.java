  ...
  static public Scene scene() throws FileNotFoundException, IOException {
    ...
    next.setOnAction(e -> {
      if (firstName.getText() != "" && lastName.getText() != ""
          ...
          && confirmPassword.getText() != "" && confirmPassword.getText()
          .equals(password.getText())) {
        Login logininfo = new Login(firstName.getText() + lastName.getText(), password.getText(),
            firstName.getText() + lastName.getText() + "#1");
        try {
          FileManagement.writeNewUser(
              new User(null, null, logininfo, email.getText(), lastName.getText(),
                  firstName.getText(), null, null));
        ...
        ApplicationObjects.alertBox("ERROR", "Missing information or password dont match",
            "Pleace fill out all required information");
      }
    });
