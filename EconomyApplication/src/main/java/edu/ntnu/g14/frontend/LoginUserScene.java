...
public class LoginUserScene {
  ...
  static public Scene scene() throws IOException {
    ...
    passwordField.setOnKeyPressed(keyEvent -> {
      ...
      if (key == KeyCode.ENTER && passwordField.getText()
          .equals(BankApplication.loggedInUser.getLoginInfo().getPassword())) {
        try {
      ...
      } else if (key == KeyCode.ENTER && !passwordField.getText()
          .equals(BankApplication.loggedInUser.getLoginInfo().getPassword())) {
    ...
    Text welcomeText = ApplicationObjects.newText(
        "Welcome back, " + BankApplication.loggedInUser.getLoginInfo().getUserName(), 25, false, 0,
        40);
