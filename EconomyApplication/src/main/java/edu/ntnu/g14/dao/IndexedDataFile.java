...
public class IndexedDataFile {
   ...
   * @return a byte array containing all the data stored under the given identifier, null if no data
   * is found
  ...
  public void addNewData(String identifier, byte[] data) throws IOException {
    if (identifier.contains(String.valueOf(IDENTIFIER_SEPARATOR))) {
      throw new IllegalArgumentException("Identifier cannot contain " + IDENTIFIER_SEPARATOR
          + ". The submitted invalid identifier was '" + identifier + "'");
    }
    if (identifier.contains(String.valueOf(DATA_SEPARATOR))) {
      throw new IllegalArgumentException(
          "Identifier cannot contain " + DATA_SEPARATOR + ". The submitted invalid identifier was '"
              + identifier + "'");
    }
   ...
   * @throws IOException              if an I/O error occurs
   * @throws IllegalArgumentException if the index is out of bounds or the identifier does not
   *                                  exist
   */
  public void setData(String identifier, int index, byte[] data) throws IOException {
    ...
    if (dataStart == -1) {
      throw new IllegalArgumentException("Could not find any data at index " + index
          + ". Either the index is out of bounds, or the identifier " + identifier
          + " does not exist.");
    }
   ...
   * @throws IOException if an I/O error occurs
   */
  public byte[][] getRangeOfEntries(String identifier, int startIndex, int endIndex)
      throws IOException {
  ...
  /**
   * Checks if the specified identifier is present in the stored data. This method opens the
   * required streams, reads all identifiers, and then closes the streams.
   *
  ...
  private byte[] convertFromStorageFormat(byte[] data) {
    ...
    for (int i = 0; i < data.length; i++) {
      if (data[i] == ESCAPE_CHAR && i + 1 < data.length
          && data[i + 1] == DATA_SEPARATOR_REPLACEMENT) {
  ...
  /**
   * Deletes the data at the specified index for the given identifier. This method updates the start
   * position index after deleting the data.
   *
   ...
   * @return The position of the specified index for the given identifier, or -1 if the identifier
  ...
  /**
   * Skips past any bytes in the fileStream until either INDEX_SEPARATOR or DATA_SEPARATOR is
   * found.
   *
   * @return The file pointer position pointing to the next INDEX_SEPARATOR or DATA_SEPARATOR, or -1
   * if end of file is reached before finding either separator, or -2 if the DATA_SEPARATOR is found
   * instead of the INDEX_SEPARATOR.
   * @throws IOException
   */
  ...
  /**
   * Appends the given data index to the entry with the specified identifier in the index section of
   * the file. If the entry does not exist, creates a new entry for the identifier.
   *
  ...
  /**
   * Reads bytes from the given stream until an IDENTIFIER_SEPARATOR is found, and returns the bytes
   * as a string.
   *
   * @param stream The stream to read from.
   * @return The bytes read as a string, or null if a DATA_SEPARATOR is found before an
   * IDENTIFIER_SEPARATOR, or end of file is reached before finding an IDENTIFIER_SEPARATOR.
   * @throws IOException
   */
   ...
   *
   * @return The data index pointing past the next line shift. If end of file was reached, returns
   * the position at the end of the file.
   * @throws IOException
   */
  private long skipLine() throws IOException {
    try {
      while (fileStream.readByte() != DATA_SEPARATOR) {
        continue;
      }
    } catch (EOFException e) {
   ...
   *
   * @return The bytes read as a byte array, or an empty byte array if end of file is reached before
   * finding a new line character.
   * @throws IOException
   */
  ...
  /**
   * Reads a line from the fileStream until a new line character is found, and returns the line as a
   * string.
   *
   * @return The line read as a string, or an empty string if end of file is reached before finding
   ...
   *
   * @param identifier The identifier to search for.
   * @return The byte position where the index entry for the identifier starts, or -1 if the
  ...
  /**
   * Reads the index line of the submitted identifier, and returns all indices as an array. An index
   * points to the start of the data.
   *
   * @return A long[] containing all indices under the submitted identifier. Returns null if no such
   * identifier exists.
   * @throws IOException
   */
  ...
  /**
   * Updates all index values that come after the specified position with the given data amount
   * change. If the data amount change is 0 or if there is no data start position set, no updates
   * will be made.
   *
  ...
  private void updateIndices(long dataAmountChange, long positionOfChange) throws IOException {
    ...
    for (long[] indexGroup : allIndices) {
      for (int i = 0; i < indexGroup.length; i++) {
        indexGroup[i] =
            indexGroup[i] > positionOfChange ? indexGroup[i] + dataAmountChange : indexGroup[i];
      }
    }
  ...
  /**
   * Reads the start position of the data in the file by searching for the double line shift that
   * marks the beginning of the data. If no double line shift is found, returns -1.
   *
  ...
  /**
   * Reads all data in the index part of the file, including the double line shift marking the
  ...
  /**
   * Reads all present identifiers from the file. If the file does not contain any identifiers,
  ...
  private long[][] getAllIndices() throws IOException {
    ...
    while (!(line = readLineS()).isEmpty()) {
      indexGroups.add(Arrays.stream(line.split(String.valueOf(IDENTIFIER_SEPARATOR))[1].split(
              String.valueOf(INDEX_SEPARATOR)))
          .map(s -> s.replaceAll("\\s+", ""))
          .mapToLong(Long::parseLong)
          .toArray());
    }
  ...
  /**
   * Opens the file and temporary streams if they are not already open. If the streams are already
   * open, increments a counter instead of reopening the streams.
   *
   * @throws IOException if there is an issue accessing the file
   */
  ...
  /**
   * Closes the file and temporary streams if the number of chained open calls is 1. If the number
   * of chained open calls is greater than 1, decrements a counter instead of closing the streams.
   *
   * @throws IOException if there is an issue accessing the file
   */
